...
class CreateJournalFormTest(TestCase):
    ...
    def test_blank_journal_bio(self):
        ...
        form = CreateJournalForm(data=self.form_input)
        self.assertFalse(form.is_valid())
        self.assertEqual(form.errors['journal_bio'], ['This field is required.'])
    ...
    def test_long_journal_description(self):
        ...
        form = CreateJournalForm(data=self.form_input)
        self.assertFalse(form.is_valid())
        self.assertEqual(form.non_field_errors(), ['Ensure this value has at most 1000 characters.'])

    def test_long_journal_bio(self):
        self.form_input['journal_bio'] = 'x' * 10001  # exceeding the limit by 1
        form = CreateJournalForm(data=self.form_input)
        self.assertFalse(form.is_valid())
        self.assertEqual(form.non_field_errors(), ['Ensure this value has at most 10000 characters.'])
