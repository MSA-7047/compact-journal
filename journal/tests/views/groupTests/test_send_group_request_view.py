...
class SendGroupRequestViewTest(TestCase):
    ...
    def test_send_group_request_not_owner(self):
        ...
        GroupMembership.objects.create(user=self.recipient, group=self.group)
    ...
    def test_reject_group_request(self):
        GroupRequest.objects.create(group=self.group, sender=self.user, recipient=self.recipient)
        self.client.force_login(self.recipient)
    ...
    def test_send_request_already_existing_request(self):
        GroupRequest.objects.create(recipient=self.recipient, group=self.group, sender=self.user)
        ...
        self.assertEqual(response.status_code, 200)
        self.assertContains(response, f"{self.recipient} has already been invited.")
    ...
    def test_send_request_already_existing_member(self):
        ...
        self.assertEqual(response.status_code, 200)
        self.assertContains(response, f"{self.recipient} is already a member.")
