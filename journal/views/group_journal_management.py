...
def create_group_journal(request, group_id):
    ...
    return render(
        request,
        "create_group_journal.html",
        {"form": form, "group_id": group_id, "is_edit": False},
    )
...
def delete_group_journal(request, group_id, journal_id):
    ...
    group_membership = get_object_or_404(
        GroupMembership, user=request.user, group=journal.owner
    )
    ...
    if request.method == "POST":
        ...
        for member in memberships:
            notif_message = (
                f"The entry {journal.title} in group '{group.name}' has been deleted."
            )
            Notification.objects.create(
                notification_type="group", message=notif_message, user=member.user
            )
        ...
        journal.delete()
        messages.success(
            request, f"The entry {journal.title} has been deleted successfully."
        )
