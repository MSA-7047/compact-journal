...
def group(request) -> HttpResponse:
    ...
    group_members = [
        GroupMembership.objects.filter(group=group).count()
        for group in current_user_groups
    ]
    groups_and_members = zip(current_user_groups, group_members)
    group_request = GroupRequest.objects.filter(
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            if ActionCooldown.can_perform_action(
                request.user, "create_group", cooldown_hours=1
            ):
                ...
                messages.success(
                    request,
                    "New group created! However, you must wait before getting points again.",
                )

            create_notification(
                request, 
                f"New group {form_data.name} created!", 
                "group"
            )
        ...
        if form.is_valid():
            ...
            for member in memberships:
                ...
                Notification.objects.create(
                    notification_type="group", message=notif_message, user=member.user
                )

            return redirect("group_dashboard", group_id=group_id)

    return render(
        request, "edit_group.html", {"form": form, "group_id": group.group_id}
    )
...
def send_group_request(request, group_id):
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            existing_member = GroupMembership.objects.filter(
                user=recipient, group=group_
            ).first()
            ...
            if existing_member:
                ...
                return render(
                    request,
                    "send_group_request.html",
                    {"form": form, "group_id": group_id},
                )
            
            existing_request = GroupRequest.objects.filter(
                sender=request.user, recipient=recipient, group=group_
            ).first()
            ...
            Notification.objects.create(
                ...
                message=f"{request.user} has invited you to a new group named {group_.name}.",
                user=recipient,
            )
    ...
    return render(
        request, "send_group_request.html", {"form": form, "group_id": group_.group_id}
    )
...
def accept_group_request(request, group_id):
    ...
    Notification.objects.create(
        notification_type="group",
        message=f"{request.user} has joined your group named {group}",
        user=group_request.sender,
    )
    Points.objects.create(
        user=group_request.sender,
        points=40,
        description=f"{request.user} has joined your group named {group}",
    )
...
def reject_group_request(request, group_id):
    ...
    Notification.objects.create(
        notification_type="group",
        message=f"{request.user} has rejected your invite to group {group}",
        user=group_request.sender,
    )
...
def delete_group(request, group_id):
    ...
    return render(
        request,
        "delete_account.html",
        {"form": form, "group_id": group.group_id, "is_group": True},
    )
...
def remove_player_from_group(request, group_id, player_id):
    ...
    if not user_membership.is_owner:
        return HttpResponseForbidden(
            "You are not authorized to remove a player from this group."
        )
...
def select_new_owner(request, group_id):
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            new_owner_membership = get_object_or_404(
                GroupMembership, group=group, user=new_owner
            )
            ...
            Notification.objects.create(
                notification_type="group",
                message=f"You are the new owner for {group.name}",
                user=new_owner,
            )
            return redirect("dashboard")
    ...
    return render(
        request,
        "select_new_owner.html",
        {"form": form, "group": group, "user": request.user},
    )
